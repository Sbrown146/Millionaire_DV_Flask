...
function paf_question_table(q, w, x, y, z, div) {
    let table_columns = [["# of Times Used", "# of Times Produced Get Correct", "# of Times Did Not Know", "% Correct When Used"], [w, x, y, `${z}%`]];
...
function outcome_question_table(q, v, u, w, x, y, z, div) {
    let table_columns = [["Total Runs", "Runs Ended", "Walked", "Games Lost",  "Walked%", "Games Lost%"], [v, u, w, x, `${y}%`, `${z}%`]];
...
function outcome_question_table_final(q, v, u, w, x, y, z, div) {
    let table_columns = [["Total Runs", "Walked", "Games Won", "Games Lost",  "Games Won%", "Games Lost%"], [v, u, w, x, `${y}%`, `${z}%`]];
...
function question_reached_graph_generate(x, y, z, div) {
    ...
    let trace = {
        ...
        type: "scatter",
        name: "Total Who Reached Question"
    };
...
function initialize_page_two() {
    ...
    d3.json("/default_data", function(error, data) {
        ...
        $("#prize_money_common").text(`The graph below showcases that a majority of contestants (${truncate((question_eleven.length/question_array.length).toFixed(4)*100, 2)}%) made it all the way to question 11 before either walking away or losing outright as the cumulative total who reached question 12 is less than those who lost or walked away for this first time on the graph.  This trend continues for question 12 – 15 (${truncate((question_twelve.length/question_array.length).toFixed(4)*100, 2)}% at question 12, ${truncate((question_thirteen.length/question_array.length).toFixed(4)*100, 2)}% at qestion 13 and so on).`);
